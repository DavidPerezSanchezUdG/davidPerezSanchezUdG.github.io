...
function initHTML(Scene) {
	for (let i = 0; i < Scene.Lights.length; i++) {
		...
		let opt = new Option(txt,i.toString());
		selectIllum.appendChild(opt);
	}
...
function onSelectShapes() {
	...
	for (let material in Materials) {
		let txt = material;
		let opt = new Option(txt,txt);
		selectShapeMaterial.appendChild(opt);
		if( JSON.stringify(Materials[material]) ===  JSON.stringify(selectedMaterial)) name = material;
	}
...
function onChangedMultithread() {
	if (checkboxMultithreading.checked) {
		...
		if(workers.length === 0) {
			for (let i = 0; i < nThreads.value; i++) {
